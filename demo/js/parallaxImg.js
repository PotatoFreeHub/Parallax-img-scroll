...
function parallaxImgScroll(settings) {
  ...
  if (parallaxSettings.pageLoader) {
    ...
    $(".parallaxImg-loading-page").prepend(
      "<div class='parallaxImg-loading-text'>Loading Page</div>"
    );
        ...
        // for all the elements that have the class "parallax-move"
          ...
          if ($(setOfElements[i]).hasData("ps-vertical-position")) {
            ...
            var TopPosition = Math.floor(
              Math.random() * (heightOfContainer - heightOfContainer / 4) + 1
            );
          }

          //if the element has an horizontal position declared
          if ($(setOfElements[i]).hasData("ps-horizontal-position")) {
            var leftPosition = $(setOfElements[i]).data(
              "ps-horizontal-position"
            );
            var rightPosition = undefined;
          } else if (
            $(setOfElements[i]).hasData("ps-horizontal-position-right")
          ) {
            var rightPosition = $(setOfElements[i]).data(
              "ps-horizontal-position-right"
            );
            var leftPosition = undefined;
          } else {
            var leftPosition = Math.floor(
              Math.random() * (widthOfContainer - 100) + 50
            );
            var rightPosition = undefined;
          }
    ...
    for (i = 0; i < parallaxElementsArray.length; i++) {
      ...
      if (isVisible(distanceFromTop, elementHeight)) {
        ...
        if (lastestScrolled < scrolled) {
          ...
          parallaxElementsArray[i].privateScrolled =
            parallaxElementsArray[i].privateScrolled +
            (scrolled - lastestScrolled);
        } else if (scrolled == 0) {
          alpha = parallaxSettings.initialOpacity;
        ...
        $(parallaxElementsArray[i].element).css({
          opacity: alpha,
          bottom:
            parallaxElementsArray[i].verticalPagePosition +
            parallaxElementsArray[i].privateScrolled *
              parallaxElementsArray[i].scrollSpeed +
            "px",
        });
